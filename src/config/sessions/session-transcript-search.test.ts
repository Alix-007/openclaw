...
describe("searchSessionTranscripts", () => {
  ...
  it("combines time bounds with generation, role, recent order and limit", async () => {
    const sessionKey = "agent:main:time-role";
    for (const [role, now] of [
      ...
      await appendTranscriptMessage(transcriptScope("time-role", sessionKey), {
        now,
        message: { role, content: [{ type: "text", text: "window needle" }] },
      });
    }
    await appendTranscriptMessage(
      transcriptScope("other-time-role", "agent:main:other-time-role"),
      {
        now: 2_900,
        message: { role: "assistant", content: [{ type: "text", text: "window needle" }] },
      },
    );
    const params = {
      agentId: "main",
      env: env(),
      query: "needle",
      sessionKeys: [sessionKey, "agent:main:other-time-role"],
      sessionId: "time-role",
      order: "recent" as const,
      minTimestampMs: 2_000,
      beforeTimestampMs: 3_000,
      limit: 1,
    };
    expect(searchSessionTranscripts({ ...params, role: "assistant" })).toMatchObject({
      hits: [{ sessionId: "time-role", role: "assistant", timestamp: 2_700 }],
      truncated: true,
    });
    expect(searchSessionTranscripts({ ...params, role: "user" })).toMatchObject({
      hits: [{ sessionId: "time-role", role: "user", timestamp: 2_000 }],
      truncated: false,
    });
  });
