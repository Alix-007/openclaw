...
const [root, sha, receipts] = process.argv.slice(2);
...
async function run(label, args, quiet = false) {
  ...
  if (!quiet) await writeFile(path.join(receipts, `${label}.json`), JSON.stringify(result, null, 2));
  return result;
}
...
try {
  ...
  for (const entry of before) {
    ...
    await writeFile(path.join(receipts, 'observations.json'), JSON.stringify(observations, null, 2));
    assert.equal(text, entry.red ?? expectedProduct, `${entry.name}: precise parent behavior`);
  }
  assert.equal(observations.filter((entry) => !entry.productBytesPreserved).length, 3);
  ...
  await writeFile(path.join(receipts, 'SUMMARY.json'), JSON.stringify({ sha, mode: 'parent',
    realGateway: true, cases: before.length, expectedViolations: 3, controlsPassed: true,
