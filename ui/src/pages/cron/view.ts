...
function conversationOptionValue(entry: CronSuggestion): string {
  ...
  const route = entry.threadId ? `${base} [thread ${entry.threadId}]` : base;
  return `${route} [account ${entry.accountId}]`;
}
        ...
        ? html`
            ...
            ${renderCronInputField(props, "deliveryTo", {
              ...
              onInput: (value) => {
                ...
                return {
                  deliveryAccountId: selected?.accountId ?? props.form.deliveryAccountId,
    ...
    ${props.form.failureAlertMode === "custom"
      ? html`
          ...
          ${renderCronInputField(props, "failureAlertTo", {
            ...
            onInput: (value) => {
              const selected = findConversation(value, props.failureToSuggestions);
              return {
                failureAlertAccountId: selected?.accountId ?? props.form.failureAlertAccountId,
                failureAlertTo: selected?.target ?? value,
              };
            },
          })}
